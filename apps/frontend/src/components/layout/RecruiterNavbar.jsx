...
export default function RecruiterNavbar() {
  ...
  useEffect(() => {
    itemRefs.current = itemRefs.current.slice(0, 3);
  }, []);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      switch (e.key) {
        case 'ArrowRight':
          e.preventDefault();
          newIndex = (activeIndex + 1) % 3;
          break;
        case 'ArrowLeft':
          e.preventDefault();
          newIndex = (activeIndex - 1 + 3) % 3;
          break;
        case 'Tab':
          if (activeIndex === 2 && !e.shiftKey) {
            e.preventDefault();
            newIndex = 0;
  ...
  return (
    <nav 
      ...
      role="navigation"
      aria-label="Navegación de reclutador"
    >
